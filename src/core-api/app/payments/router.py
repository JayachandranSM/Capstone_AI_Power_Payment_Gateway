...
async def run_fraud_engine(sender_id: uuid.UUID, amount: Decimal,
                            currency: str, method: str, db: AsyncSession) -> tuple[Decimal, list]:
    score = Decimal("0.0")
    rules_hit = []

    # Load weights from DB (feedback loop)
    weights_row = await db.execute(
        text("SELECT rule_name, weight FROM ai.fraud_patterns")
    )
    weights = {r.rule_name: Decimal(str(r.weight)) for r in weights_row.fetchall()}

    def w(rule: str, default: float) -> Decimal:
        return weights.get(rule, Decimal(str(default)))

    def hit(rule: str, detail: str, weight_key: str, default: float):
    ...
    # Rule 2: New account
    row = await db.execute(
        text("SELECT created_at FROM core.users WHERE id = :uid"), {"uid": sender_id}
    )
    user = row.fetchone()
    if user:
    ...
    # Rule 3: High velocity (>5 txns in 10 min)
    v_row = await db.execute(
        text("""SELECT COUNT(*) as cnt FROM ledger.transactions
                WHERE sender_id=:uid AND created_at > NOW() - INTERVAL '10 minutes'"""),
        {"uid": sender_id},
    )
    v = v_row.fetchone()
    if v and v.cnt > 5:
    ...
    # Rule 9: Night transaction (11pm-5am IST)
    hour = datetime.now(timezone.utc).hour
    if hour >= 17 or hour <= 23:  # UTC offset for IST late night
        score += hit("NIGHT_TRANSACTION", "Late night transaction", "NIGHT_TRANSACTION", 0.1)

    # Rule 10: Round amount (large)
    if amount >= 1000 and amount % 1000 == 0:
        score += hit("ROUND_AMOUNT", "Large round amount", "ROUND_AMOUNT", 0.1)

    # Update hit counts in DB for feedback loop
    for r in rules_hit:
        await db.execute(
            text("""UPDATE ai.fraud_patterns
                    SET hit_count = hit_count + 1, last_updated = NOW()
...
async def process_payment(payload: dict,
    ...
    log.info("payment_initiated", user_id=str(current_user.user_id),
    ...
    if payload.get("receiver_upi"):
        ...
        if not handle:
            raise HTTPException(status_code=404,
                                detail=f"UPI handle '{payload['receiver_upi']}' not found")
    ...
    # Check wallet with optimistic lock
    wallet_row = await db.execute(
        text("""SELECT id, balance, locked_balance, status, version
                FROM core.wallets
                WHERE user_id=:uid AND currency=:cur FOR UPDATE"""),
        {"uid": current_user.user_id, "cur": currency},
    )
    wallet = wallet_row.fetchone()
    if not wallet:
        raise HTTPException(status_code=404,
                            detail=f"No {currency} wallet. Please top up first.")
    ...
    if available < amount:
        raise HTTPException(status_code=400,
                            detail=f"Insufficient balance. Available: {available} {currency}")

    # FX + fraud
    fx_rate    = await get_fx_rate(currency, "USD", db)
    amount_usd = amount * fx_rate
    fraud_score, rules_hit = await run_fraud_engine(
        current_user.user_id, amount, currency, method, db
    )

    # Chargeback probability (simple ML proxy)
        ...
        text("""
            ...
            VALUES
                (:id, :idem, :sender, :receiver, :merchant,
                 ...
                 'sandbox', :sandbox_ref, :metadata::jsonb)
        """),
        {
            "id": tx_id, "idem": idem_key,
            "sender": current_user.user_id, "receiver": receiver_id,
            "merchant": payload.get("merchant_id"),
            "amount": amount, "currency": currency,
            "amount_usd": amount_usd, "fx_rate": fx_rate,
            "type": tx_type, "status": tx_status, "method": method,
            "upi_sender": sender_upi.handle if sender_upi else None,
            "upi_receiver": upi_receiver,
            "failure": failure_reason, "fraud_score": fraud_score,
    ...
    if tx_status == "success":
        await db.execute(
            text("""UPDATE core.wallets
                    ...
                    WHERE id = :wid AND version = :ver"""),
            {"amount": amount, "wid": wallet.id, "ver": wallet.version},
        )
        new_bal = Decimal(str(wallet.balance)) - amount
        await db.execute(
            text("""INSERT INTO ledger.entries
                    (transaction_id, wallet_id, direction, amount, currency, balance_after)
                    VALUES (:tx, :wid, 'debit', :amt, :cur, :bal)"""),
        ...
        if rw:
            await db.execute(
                text("""INSERT INTO ledger.entries
                        (transaction_id, wallet_id, direction, amount, currency, balance_after)
                        VALUES (:tx, :wid, 'credit', :amt, :cur, :bal)"""),
    ...
    if fraud_score >= Decimal("0.5"):
        severity = ("critical" if fraud_score >= Decimal("0.85") else
                    "high"     if fraud_score >= Decimal("0.70") else
                    "medium"   if fraud_score >= Decimal("0.50") else "low")
        await db.execute(
            text("""INSERT INTO ai.fraud_alerts
                    (transaction_id, fraud_score, severity, rules_triggered)
                    VALUES (:tx, :score, :sev, :rules::jsonb)"""),
            {"tx": tx_id, "score": fraud_score, "sev": severity,
             "rules": json.dumps(rules_hit)},
        )
    ...
    await db.execute(
        text("""INSERT INTO ops.notifications (user_id, type, title, body, metadata)
                VALUES (:uid, 'payment', :title, :body, :meta::jsonb)"""),
        {
            "uid": current_user.user_id, "title": notif_title,
    ...
    await db.commit()
    log.info("payment_processed", tx_id=str(tx_id), status=tx_status,
